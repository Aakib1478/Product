...
class Trade:
    def __init__(self,expert_name,version,symbol,magic_number,lot: float,stop_loss,emergency_stop_loss,take_profit,emergency_take_profit,start_time='0:10',finishing_time='23:59',ending_time='23:59',fee=0.0):
        ...
        # fee is nothing but spread
        self.fee = 0
        # digit is number of decimal places. will be used along with spread to calculcate stop loss and profit level
        self.digits = 0
    ...
    def initialize(self):
        if not Mt5.initialize(login=login, server=server, password=password):
            ...
            # get spread and set self.fee
            try:
                print(f'fetching spread and digits of {self.symbol}')
                symbol_info=Mt5.symbol_info(self.symbol)
                print('symbol_info')
                self.fee = symbol_info.spread
                self.digits = symbol_info.digits
            except:
                print(f'Could not fetch spread and digits of {self.symbol}')
                Mt5.shutdown()
                print('Turned off')
                quit()
    ...
    def open_position(self, buy, sell, comment=""):
        # IRFAN - commented out the trading_time method to ignore timing restrictions
        ...
        # IRFAN - review this method
        # self.stop_and_gain(comment)

        # IRFAN - commenting out the time based trade exit for time being
        # if self.days_end():
